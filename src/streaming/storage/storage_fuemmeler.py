"""src/streaming/storage/storage_fuemmeler.py.
...
def log_storage_summary(db_path: Path) -> None:
    ...
    with duckdb.connect(str(db_path)) as conn:
        ...
        rows = conn.execute(sql_by_payment_method).fetchall()
    ...
    for payment_method, sale_count, total_sales, avg_sale in rows:
        LOG.info(
            f"  {payment_method}: {sale_count}, Total Sales: {total_sales}, Average Sale: {avg_sale}"
        )
